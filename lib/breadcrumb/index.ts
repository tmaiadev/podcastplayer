import type { BreadcrumbParams, BreadcrumbItem, BreadcrumbOrigin } from './types';
...
export function parseBreadcrumbParams(
  ...
  const validOrigins: BreadcrumbOrigin[] = ['home', 'category', 'search', 'subscriptions'];
  const validFrom = from && validOrigins.includes(from) ? from : undefined;

  // Parse catId as number
  const catId = typeof catIdRaw === 'string' ? parseInt(catIdRaw, 10) : undefined;
  const validCatId = catId && !isNaN(catId) && catId >= 1 && catId <= 112 ? catId : undefined;
